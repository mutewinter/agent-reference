...
- **The examples are shared with the README.** Titles, notes, file labels, snippets, and the setup prompt live in [code-samples.mjs](./code-samples.mjs); this page maps over `examples`, and `scripts/sync-readme.mjs` renders the same array into the marked regions of [the README](../README.md), where a test fails if it is behind. Prose is written twice on purpose, because a landing page and a README are different targets; the snippets are not, because a config example with a stale key is wrong rather than differently worded.
