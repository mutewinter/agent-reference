...
- **This page is the source the README renders from.** Every word the page says about itself lives in [code-samples.ts](./code-samples.ts): the tagline and meta description, the two get-started headings, the examples, and the section copy. `scripts/sync-readme.ts` renders them, plus the transcripts from [cli-reference.ts](./cli-reference.ts), into the marked regions of [the README](../README.md), and a test fails when it is behind. Change the copy here and run `npm run sync-readme` at the root. The README's reference sections below `## Configure` are written for a README and have no counterpart here.
