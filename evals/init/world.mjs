...
export const EXPECTED = {
  ...
  noise: [
    ...
    { name: 'Downloads', sessions: 1, why: 'a screenshot, not a source of truth' },
    {
      name: 'src/components',
      sessions: 6,
      why: 'ordinary work in this repo, never "go read this"',
    },
  ],
};
...
const SESSIONS = [
  {
    ...
    turns: [
      user(
        'Can you make the product card match what is in the design system? It is at {HOME}/code/acme/design-system, look at packages/tokens for the spacing scale.',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'The checkout total is off by a cent. Compare our rounding with how platform-api does it, that is in ../platform-api under src/billing.',
      ),
      bash(
        'sed -n "1,40p" ../platform-api/src/billing/round.ts',
        'export function roundMinor(value: number): number {',
      ),
      say(
        'platform-api rounds in minor units before summing. We sum floats first. That is the cent.',
      ),
    ],
  },
  {
    ...
    turns: [
      user(
        'I want the router here to behave like tiny-router does with nested layouts. {HOME}/code/reference/tiny-router',
      ),
  ...
  },
  {
    ...
    turns: [
      ...
      bash(
        'ls ../../acme/design-system 2>/dev/null || ls {HOME}/code/acme/design-system',
        'packages\nREADME.md',
      ),
      say(
        'Found it at {HOME}/code/acme/design-system. The variants are in packages/ui/Button.tsx.',
      ),
    ],
  },
  {
  ...
  },
  {
    ...
    turns: [
      user(
        'How does chess-engine handle the transposition table? {HOME}/code/reference/chess-engine',
      ),
      bash(
        'rg -n "transposition" {HOME}/code/reference/chess-engine/src',
        'src/tt.rs:12:pub struct Transposition',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'Check https://github.com/acme/wire-format for how they version the envelope before you touch our serializer.',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'The design tokens changed again, resync from {HOME}/code/acme/design-system/packages/tokens.',
      ),
      bash(
        'diff -r {HOME}/code/acme/design-system/packages/tokens src/tokens',
        'Only in src/tokens: legacy.ts',
      ),
      say(
        'Two tokens were renamed and legacy.ts is ours alone. Updated src/components accordingly.',
      ),
    ],
  },
  {
    ...
    turns: [
      ...
      bash(
        'cat ../platform-api/src/errors.ts',
        'export interface WireError { code: string; retryable: boolean }',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'I dropped a screenshot at {HOME}/Downloads/cart-bug.png, look at it and tell me what is wrong.',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'Look at how chess-engine sets up its tests, {HOME}/code/reference/chess-engine/tests. I want that shape here.',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'Read docs/adr/0007 before touching the queue, and then tell me if the retry change is allowed.',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'wire-format again, github.com/acme/wire-format, read the changelog and tell me if v3 breaks us.',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'There is an idea I want to steal in {HOME}/code/reference/legacy-parser, the error recovery bit.',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'The design system bumped its tokens package. Diff {HOME}/code/acme/design-system against what we vendored and tell me what moved.',
      ),
      bash(
        'git -C {HOME}/code/acme/design-system log --oneline -5',
        'a1b2c3d tokens: rename surface scale',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'One more from the design system: the elevation tokens. {HOME}/code/acme/design-system/packages/tokens/elevation.ts',
      ),
      bash(
        'cat {HOME}/code/acme/design-system/packages/tokens/elevation.ts',
        'export const elevation = { flat: 0, raised: 1 }',
      ),
  ...
  },
  {
    ...
    turns: [
      user(
        'tiny-router has a nicer error when a route is missing. {HOME}/code/reference/tiny-router/src/resolve.ts',
      ),
      bash(
        'sed -n "1,30p" {HOME}/code/reference/tiny-router/src/resolve.ts',
        'throw new RouteNotFound(pathname, candidates)',
      ),
...
async function writeTranscripts(home, projectRoot) {
  const resolve = (value) => (typeof value === 'string' ? value.replaceAll('{HOME}', home) : value);
...
function bash(command, output) {
  return [
    ...
    },
    {
      ...
      message: {
        role: 'user',
        content: [{ tool_use_id: 'toolu_x', type: 'tool_result', content: output }],
      },
    },
  ];
}
