...
test('normalizes npm repository URL shapes to cloneable git URLs', () => {
  assert.equal(normalizeGitRepositoryUrl('git+https://github.com/facebook/react.git'), 'https://github.com/facebook/react.git');
  assert.equal(normalizeGitRepositoryUrl('git@github.com:colinhacks/zod.git'), 'https://github.com/colinhacks/zod.git');
  ...
  assert.equal(normalizeGitRepositoryUrl('https://gitlab.com:8443/a/b.git'), 'https://gitlab.com:8443/a/b.git');
  assert.equal(repositoryUrlFromManifestRepository('github:preactjs/preact'), 'https://github.com/preactjs/preact.git');
  assert.equal(
    repositoryUrlFromManifestRepository({ type: 'git', url: 'https://github.com/vitest-dev/vitest' }),
