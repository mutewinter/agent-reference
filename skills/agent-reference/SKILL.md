...
description: Locate and use local package, git, and folder references materialized by the agent-reference CLI. Use when a repo has agent-reference.json, agent-reference.local.json, or .agent-reference/manifest.json, or when the user asks to inspect cloned dependency source or configured agent references.
---
...
- Prefer status output over `.agent-reference/manifest.json`; the manifest is generated state from the last clone run.
