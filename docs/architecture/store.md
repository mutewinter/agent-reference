...
- `log/usage.jsonl`: one line per run of the CLI, across every project on this machine, rotating to `usage.1.jsonl` past 2 MB. `activity` counts it. It is written to the default store even when a project's `cacheDir` moves everything else, because it spans projects; `$AGENT_REFERENCE_STORE_DIR` moves it with the rest of the store. Local, sent nowhere, and the one zone deleting costs something: the history, which nothing rebuilds and nothing else reads.
