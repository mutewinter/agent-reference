...
1. The lockfile gives `name@version` (four formats: PNPM, npm, Bun text, Yarn).
...
when the funnel fails, which commit to pin (with a description saying why). The tool
computes what: the funnel is deterministic, cheap in code, and identical on every run,
