...
Status: bugs 1 through 7 fixed, the eval harness fixed, both broken fixtures rebuilt, duplicate keys refused, three decisions taken. What is left is one coverage gap, one decision record, and two small ones, marked per item below. Everything here came out of reviewing [the one-references-map change](../../decisions/2026-08-25-one-references-map.md) and running all four eval suites against it. The repo was green as found, so none of this was caught by an existing gate.
...
- **The `adopt` fixture.** The published bundle's four primitives are faithful now, holding a query, filtering against it and selecting on click, with only `exports.Combobox` inert; a bundle stubbed throughout made "the whole install is broken" the correct reading and put the graded check out of reach. `src/App.tsx` gives the provider a root to land in rather than forcing it inside the form, and `AGENTS.md` no longer announces an `agent-reference.json` the world never writes.
