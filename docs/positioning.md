...
| 3 | It costs nothing until the agent asks | Every line of an instructions file is in context on every turn | 3,717 bytes of stub on disk; the guide, the reference list, and the source itself load only on request | "One paragraph in context. Thirty-four gigabytes on demand." |
