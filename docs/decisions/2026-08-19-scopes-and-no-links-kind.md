...
and overrides same-named entries. Both files are optional.

The guardrail keys on what a value means, not on which key holds it: no kind is banned
from the committed file, because a repo-relative folder means the same thing on every
checkout and belongs there. `validate` errors on an absolute or `~/` path anywhere one can
hide, which is a `folders` path, a `file:` repository under `git`, and `cacheDir`, and
warns on `../` escapes. It also errors when `agent-reference.local.json` is tracked by
git, asked of the index rather than of `check-ignore`, which excludes tracked files and so
reports a committed config as merely unignored. `status` repeats the path checks as
warnings, since they are string work over a config it has already loaded, and an agent
runs `status` far more often than it runs `validate`.
...
- The same leak is an error in `validate` and a warning in `status`, deliberately: it
  fails a build, but it blocks no reference, so status must not tell an agent to stop.
